...
QUE REGISTRA
  ...
  2. La pendiente de CADA actuador sobre CADA tap, con un escalon chico. Cuatro
     por cuatro, con tres puntos cada uno.
...
Con la cadena railada, varias de las dieciseis pendientes van a salir cero,
...
def correr(pga=CAMPO_PGA, pgaout=CAMPO_PGAOUT, log=print):
    ...
    try:
        ...
        for etapa in range(4):
            puntos_por_tap = {ch: [] for ch in range(4)}
            ...
            for ch in range(4):
                ...
                if m is None:
                    log("   %-8s %-8s %14s %16s" % (
                        NOMBRES[etapa], "ch%d %s" % (ch, NOMBRES[ch]),
                        "-", "sin pendiente medible"))
                else:
                    log("   %-8s %-8s %14.4f %16.0f" % (
                        NOMBRES[etapa], "ch%d %s" % (ch, NOMBRES[ch]), m, uv))
    finally:
        c.close()
    ...
    log("  factor que la resistencia; las otras quince, no. Si cambia alguna que")
    ...
    ruta = guardar("t0_linea_base",
                   {"pga_x": GANANCIA[pga], "pgaout_x": GANANCIA[pgaout],
                    "reposo_mv": {str(k): reposo.get(k) for k in range(4)},
